    ...
    SECTION("CFRunLoop threads", "[run loops]") {
...

        {
            std::thread([&](){
                ...
                auto token = obj.observe([&](auto change) {
...

        {
            std::thread([&](){
...

                {
                    ...
                    auto r = realm::open<AllTypesObject, AllTypesObjectLink, AllTypesObjectEmbedded>(std::move(c));
                    auto o = r.objects<AllTypesObject>().where([](auto& obj) { return obj._id == 123; })[0];
