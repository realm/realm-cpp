...
//template <typename T>
//using BinaryPersistableListConcept = std::is_same<T, std::vector<std::vector<std::uint8_t>>>;
//template <typename T>
//using BinaryPersistableList = typename std::enable_if<BinaryPersistableConcept<T>::value>::type;
...
using NonOptionalPersistableConcept =
        std::disjunction<PrimitivePersistableConcept<T>, ObjectBasePersistableConcept<T>>;
template <typename T>
...
constexpr auto strip_list_check() {
    if constexpr (!PrimitivePersistableConcept<T>::value && !ObjectBasePersistableConcept<T>::value) {
...
template <typename T>
using ListPersistableConcept = typename std::conditional<
...
template <typename T>
// TODO: Figure out how to short circuit :: from primitive types
...
template <typename T>
using NonContainerPersistable = typename std::enable_if<
        NonOptionalPersistableConcept<T>::value || OptionalPersistableConcept<T>::value
>::type;
template <typename T>
